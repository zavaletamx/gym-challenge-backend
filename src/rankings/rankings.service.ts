...
export class RankingsService {
  ...
  async getYearly() {
    ...
    const map = new Map<number, { userId: number; name: string; count: number; lastCheckin: Date }>();
    for (const c of checkins) {
      if (!map.has(c.userId)) {
        map.set(c.userId, { userId: c.userId, name: c.user.name || 'Unknown', count: 0, lastCheckin: c.createdAt });
      }
